...
- **Honest attribution:** `generate_prose` returns `(text, degraded, stats)` — the `stats` dict carries the MSHA safety counts (`fatalities`, `injuries_lost_time`, `days_lost`, `incidents`) that the frontend's toll block surfaces at the top of section 2, independent of whether the prose itself is Cortex-written or templated. `generate_h3_summary` returns `(text, degraded)`. Only Cortex output is cached — template fallbacks are not, so a recovered model shows up on the next request and the "Cortex, on this map" / "Cortex, reading the record" bylines never sit over template prose.
...
- `H3Density`: resolution-5 hexbins, bigger dot = more mines, rust→ash gradient for active→abandoned. SQL filters null-island and ocean outliers at the query layer. Single map frame fit tight on the hex cluster ("the shape of extraction"). The eGRID subregion polygon framing lives upstream on `MapSection` (N° 03) so each section owns one framing instead of hedging between two. A labeled MINE card anchors the reader's dot in the cluster (same helper, same subtitle format as section 03); the user pin stays label-free on purpose — a card over the cluster would block the shape it's trying to show. Render failures surface in the console with a full stack and flip a single `errored` flag; the outer data-fetch catch is the only path that degrades the whole section.
...
- The **three-line anchor pattern** via `:global(.anchor-primary)` + `:global(.anchor-secondary)`: big value / serif plain-English primary / mono uppercase tag. Used by PlantReveal cards, the miner-toll block (MSHA safety stats at the top of section 2 — injuries first, fatalities second, zero-value cards omitted; block hidden entirely if every stat is zero), the land-disturbed block, the emissions panel, and the H3 tallies. **Do not re-declare these per section.** If you need a larger closing beat (Ticker), define a distinct class — don't shadow the canonical one.
- **Section content widths.** `SectionRail`'s `.rail-content` caps at `min(1040px, 100%)` so every post-hero section ends at the same right edge — without this clamp, CortexChat's form + results table sprawled to the viewport while Ticker's narrower prose caps read as a left column. Inside that column, individual blocks may still set a tighter reading measure (PlantReveal's prose at `min(820px, 100%)`, canonical `.sub` at `640px`); what's gone is per-section outer width drift. Headlines fill the content column as an edge-to-edge editorial beat. The old per-section `.map-header` / `.h3-header` wrappers have been consolidated into `.section-header`; do not reintroduce per-file copies, and do not re-add narrow `.closing` / `.dedication` caps on Ticker — those regressed the column unification.
