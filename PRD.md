...
| ~~Which Cortex LLM model for COMPLETE?~~ | Ashley | **Resolved** — Gemini removed. Cortex Complete re-introduced on `llama3.3-70b` for short safety prose (eulogy-style, injuries-first, fatalities-second). ~40x cheaper than `openai-gpt-5.2`. Cortex Analyst drives the `/ask` NL→SQL chat. |
